{{-- resources/views/enseignant/MesModules/index.blade.php --}}
...
<div x-data class="max-w-7xl mx-auto py-10 px-4">
    ...
    <div class="mb-8">
        ...
        <p class="text-sm text-gray-500 mt-1">
            Vous gérez {{ $modules->count() }} modules répartis sur {{ $modulesByFormation->count() }} formations.
        </p>
    </div>

    {{-- ↪ Accordéons Formation --}}
    <div class="space-y-6">
        @forelse ($modulesByFormation as $formationName => $mods)
            <div x-data="{ open:true }" class="rounded-xl overflow-hidden shadow-sm border border-gray-200 bg-white/70 backdrop-blur">
                ...
                <button @click="open=!open"
                    ...
                    <span class="text-lg font-semibold text-gray-800 flex items-center gap-2">
                        ...
                        {{ $formationName }}
                        <span class="text-gray-500 text-sm">({{ $mods->count() }} modules)</span>
                    </span>
                        ...
                        <div class="group relative rounded-xl border border-gray-200 bg-white p-5 shadow-sm hover:shadow-md transition-all duration-200 hover:-translate-y-0.5">
                            ...
                            <div class="mt-4 flex flex-wrap gap-2 text-xs">
                                <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-blue-50 text-blue-700">
                                    <i class="ri-file-list-3-line text-[13px]"></i> {{ $supports }} supports
                                </span>

                                {{-- Icônes types présents / manquants --}}
                                <span class="inline-flex items-center gap-1">
                                    <i class="ri-book-open-line {{ $hasCours    ? 'text-green-600' : 'text-red-500' }}"></i>
                                    <i class="ri-pencil-ruler-2-line {{ $hasTd   ? 'text-green-600' : 'text-red-500' }}"></i>
                                    <i class="{{ $hasSolution ? 'ri-checkbox-circle-line text-green-600' : 'ri-checkbox-blank-circle-line text-red-500' }}"></i>
                                </span>

                                @if($commentsUnread)
                                    <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-red-50 text-red-600">
                                        <i class="ri-chat-3-line text-[13px]"></i> {{ $commentsUnread }} non lus
                                    </span>
                                @endif

                                @isset($avgRating)
                                    <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-yellow-50 text-yellow-700">
                                        <i class="ri-star-line text-[13px]"></i> {{ number_format($avgRating,1) }}/5
                                    </span>
                                @endisset
                            </div>

                            {{-- Barre progression types requis --}}
                            <div class="mt-5">
                                <div class="flex justify-between text-[11px] text-gray-500 mb-1">
                                    <span>{{ $progress }} % des types requis</span>
                                </div>
                            ...
                            @if($lastSupportTitle)
                                <p class="mt-4 text-xs text-gray-500">
                                    Dernier support : <span class="text-gray-700">{{ $lastSupportTitle }}</span>
                                    @if($lastSupportDate) · {{ $lastSupportDate->format('d/m/Y') }} @endif
                                </p>
                            @endif
